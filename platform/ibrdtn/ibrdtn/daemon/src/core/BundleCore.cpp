	...
	namespace core
	{
		...
		void BundleCore::raiseEvent(const dtn::net::TransferCompletedEvent &completed) throw ()
		{
					...
					&& (meta.procflags & dtn::data::Bundle::DESTINATION_IS_SINGLETON))
			{
				...
				IBRCOMMON_LOGGER_TAG("BundleCore", notice) << "singleton bundle delivered: " << meta.toString() + " dst=" + meta.destination.getNode().getString() << IBRCOMMON_LOGGER_ENDL;
		...
		void BundleCore::inject(const dtn::data::EID &source, dtn::data::Bundle &bundle, bool local)
		{
			...
			try {
				if (local)
				{
					...
					IBRCOMMON_LOGGER_TAG(TAG, notice) << "Bundle received " + bundle.toString() + " dst=" + bundle.destination.getNode().getString() + " (local)" << IBRCOMMON_LOGGER_ENDL;
