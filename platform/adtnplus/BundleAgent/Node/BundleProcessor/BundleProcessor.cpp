...
void BundleProcessor::receiveBundles() {
  ...
  if (!s) {
    ...
    if (!s.bind(m_config.getNodeAddress(), m_config.getNodePort())) {
      // Stop the application
      LOG(1) << "Cannot bind socket to " << m_config.getNodeAddress() <<":"<< m_config.getNodePort()
...
#ifdef LEPTON
            // TODO lepton
            // add this line for log parsing
            LOG(6) << "New Bundle created " << b->getId() << " " 
              << b->getPrimaryBlock()->getSource() << "@" << b->getPrimaryBlock()->getDestination() ; 
#endif
          }
          LOG(42) << "Creating bundle container";
          // Create the bundleContainer
          std::unique_ptr<BundleContainer> bc = createBundleContainer(std::move(b));
...
void BundleProcessor::forward(Bundle bundle, std::vector<std::string> nextHop) {
 ...
 // Bundle length, this will limit the max length of a bundle to 2^32 ~ 4GB
  ...
  if (bundleLength <= 0) {
                    ...
                    if (!(s << bundleLength)) {
                      ...
                      if (!(s >> ack)) {
                        ...
                        if (ack == static_cast<uint8_t>(BundleACK::CORRECT_RECEIVED) ||
                            ack == static_cast<uint8_t>(BundleACK::QUEUE_FULL) ||
                            ack == static_cast<uint8_t>(BundleACK::ALREADY_IN_QUEUE)) { // TODO add this line beacause it's not necessary to resend a bundle which is allready in queue.
...
#endif
                          s.close(); // Ajout	because sockets needs to be closed before throwing an exception	############################################
    ...
    std::vector<std::string> minNextHop = m_neighbourTable->getMinNeighbours(nextHop); // modif shown in another branch on the git of ADTNPLUS, to get only the min neighbours ############################################
